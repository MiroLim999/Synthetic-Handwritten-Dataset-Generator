...
def create_custom_augmentation_profile(
    *,
    ...
    return AugmentationProfile(
        ...
        fade_probability=0.90 if fade_contrast < 0.95 else 0.0,
        fade_range=(fade_contrast * 0.85, fade_contrast),
        brightness_probability=0.50 if noise_std > 0 else 0.0,
        brightness_range=(0.75, 1.10),
        paper_tint_probability=0.95 if paper_tint_alpha > 0 else 0.0,
        paper_tint_alpha_range=(paper_tint_alpha * 0.8, paper_tint_alpha),
        stain_probability=stain_prob,
        rotate_probability=0.90 if rotate_deg > 0 else 0.0,
        rotate_degrees=rotate_deg,
        blur_probability=0.90 if blur_radius > 0 else 0.0,
        blur_radius_range=(blur_radius * 0.7, blur_radius),
        noise_probability=0.90 if noise_std > 0 else 0.0,
        noise_std_range=(noise_std * 0.5, noise_std),
        paper_texture_probability=0.90 if paper_texture_std > 0 else 0.0,
        paper_texture_std_range=(paper_texture_std * 0.5, paper_texture_std),
        scanline_probability=0.90 if scanline_alpha > 0 else 0.0,
        scanline_alpha_range=(max(0, scanline_alpha - 4), scanline_alpha),
        compression_probability=0.90 if jpeg_quality < 100 else 0.0,
        jpeg_quality_range=(max(10, jpeg_quality - 10), jpeg_quality),
    )
